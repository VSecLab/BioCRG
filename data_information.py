...
DATA_PATH = "/Users/grims/Documents/Research/Tesi/ML_tesi/data_logs/raw_original"
...
def calculate_activity_averages(merged_entries):
    ...
    for act, all_entries in activity_averages.items():
        if all_entries:
            ...
            # Different filtering options for minimum calculation
            # Change the filter_choice to test different approaches:
            # "25_percent", "50_percent", "percentile_20", "time_3sec", "none"
            filter_choice = "25_percent"
            
            if filter_choice == "25_percent":
                # Use 25% of average as threshold (less stringent)
                threshold = avg_entries * 0.25
                valid_entries = [e for e in all_entries if e >= threshold]
                filter_name = "25% of average"
            elif filter_choice == "50_percent":
                # Use 50% of average as threshold 
                threshold = avg_entries * 0.5
                valid_entries = [e for e in all_entries if e >= threshold]
                filter_name = "50% of average"
            elif filter_choice == "percentile_20":
                # Use percentile (exclude bottom 20%)
                sorted_entries = sorted(all_entries)
                percentile_20_idx = int(len(sorted_entries) * 0.2)
                valid_entries = sorted_entries[percentile_20_idx:]
                filter_name = "exclude bottom 20%"
            elif filter_choice == "time_3sec":
                # Use minimum time threshold (3 seconds = 150 entries)
                threshold = 150  # 3 seconds at 50 entries/sec
                valid_entries = [e for e in all_entries if e >= threshold]
                filter_name = "min 3 seconds"
            else:  # "none"
                valid_entries = all_entries
                filter_name = "no filter"
            
            if valid_entries:
                min_valid_entries = min(valid_entries)
                min_valid_time_seconds = min_valid_entries / sampling_rate
                valid_count = len(valid_entries)
                # Calculate average for filtered entries
                avg_valid_entries = sum(valid_entries) / len(valid_entries)
                avg_valid_time_seconds = avg_valid_entries / sampling_rate
            else:
                # Fallback to overall minimum if no entries meet criteria
                min_valid_entries = min(all_entries)
                min_valid_time_seconds = min_valid_entries / sampling_rate
                valid_count = len(all_entries)
                avg_valid_entries = avg_entries
                avg_valid_time_seconds = avg_time_seconds
                filter_name = "no filter (fallback)"
            
            averages[act] = {
                'average': avg_entries,
                'avg_valid': avg_valid_entries,
                'total_executions': len(all_entries),
                'min_entries': min(all_entries),
                'max_entries': max(all_entries),
                'min_valid_entries': min_valid_entries,
                'valid_executions_count': valid_count,
                'filter_type': filter_name,
                'avg_time_seconds': avg_time_seconds,
                'avg_valid_time_seconds': avg_valid_time_seconds,
                'avg_time_minutes': avg_time_seconds / 60,
                'avg_valid_time_minutes': avg_valid_time_seconds / 60,
                'min_time_seconds': min(all_entries) / sampling_rate,
                'min_valid_time_seconds': min_valid_time_seconds,
...
def main():
    ...
    print("\nAverage entries per execution by activity (all users):")
    for act, stats in activity_averages.items():
        print(f"{act}:")
        print(f"  Average entries per execution (all): {stats['average']:.1f}")
        print(f"  Average entries per execution (filtered): {stats['avg_valid']:.1f}")
        print(f"  Average execution time (all): {stats['avg_time_seconds']:.1f} seconds ({stats['avg_time_minutes']:.2f} minutes)")
        print(f"  Average execution time (filtered): {stats['avg_valid_time_seconds']:.1f} seconds ({stats['avg_valid_time_minutes']:.2f} minutes)")
        print(f"  Total executions: {stats['total_executions']}")
        print(f"  Valid executions (filter: {stats['filter_type']}): {stats['valid_executions_count']}")
        print(f"  Min entries in single execution: {stats['min_entries']} ({stats['min_time_seconds']:.1f} seconds) [all executions]")
        print(f"  Min entries (filtered): {stats['min_valid_entries']} ({stats['min_valid_time_seconds']:.1f} seconds)")
