...
Based on run_model.py but adapted to avoid intermediate file writes.
"""
...
def perform_clustering(df, df_origin, clustering_algorithm, k_value, eps, min_samples, 
    ...
    No file saving - all data kept in memory.
...
def compute_transitions_from_dataframes(adj_df, labeled_data_df, enable_plots=False):
    """
    Compute transition matrix and probabilities for Markov Chain using in-memory dataframes.
    
    Args:
        adj_df: Adjectives DataFrame
        labeled_data_df: DataFrame with all points and labels
        enable_plots: Whether to enable plotting
    
    Returns:
        tuple: (df_transition, df_probs, df_initial, sequence_stats)
    """
    # Use the transition_matrix_computation function from transition.py
    df_transition, df_probabilities, df_initial, sequence_stats = tr.transition_matrix_computation(
        adj_df, labeled_data_df
    )
    
    if enable_plots:
        # print("\n== Transition Matrix: ==")
        tr.heatmap_plot(df_transition)
        # print("\n== Transition Probabilities: ==")
        tr.heatmap_plot_probabilities(df_probabilities)
...
def segment_test_data(test_activity, features, scaler, new_threshold, process_id=None):
    ...
    for file in files: 
        ...
        # print(f"[{process_id}] Segmentating user: {username}, test_activity: {test_activity}, scaler: {scaler}, threshold: {new_threshold} =====")
...
def save_results(df_sequence_ghost_end, features, target_activity, test_activity, threshold, 
    ...
    path = config.RESULTS_DIR + f"/recognition_results/results_markov_3.csv"
...
def run_model_pipeline(
    ...
    Args:
        ...
        process_id: Process ID for multiprocessing (default: None)
    ...
    print(f"Process id: {process_id} - Starting segmentation for training data.")
    ...
    # Perform clustering (no file saving)
    print(f"Process id: {process_id} - Starting clustering for training data.")
    ...
    print(f"Process id: {process_id} - Starting creation of substantives and adjectives for training data.")
    ...
    # Compute transitions (using in-memory dataframes instead of files)
    print(f"Process id: {process_id} - Starting transition matrix computation for training data.")
    df_transition, df_probs, df_initial, sequence_stats = compute_transitions_from_dataframes(
        df_adjectives, labeled_data_df, enable_plots
    )
    
    # Segment test data (using same threshold as training)
    print(f"Process id: {process_id} - Starting segmentation for test data.")
    results = segment_test_data(test_activity, features, scaler, threshold, process_id=process_id)
    rsv_df_test, lsv_users = results
    
    # Classify test points (using in-memory labeled_data_df)
    print(f"Process id: {process_id} - Starting classification of test points.")
    ...
    print(f"Process id: {process_id} - Starting computation of user sequences and probabilities using Markov Chain.")
    df_sequence_ghost_end = compute_user_states_and_probabilities(
        lsv_users, log_df, df_adjectives, df_probs, df_initial,
        sequence_stats, ghost_exp, test_activity
    )
    ...
    results = {
        ...
        'transition_probs': df_probs,
        'initial_probs': df_initial,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run model pipeline using Markov Chain (no intermediate file I/O)')
    ...
    print(f"  - transition_matrix: {results['transition_matrix'].shape}")
    print(f"  - transition_probs: {results['transition_probs'].shape}")
    print(f"  - initial_probs: {results['initial_probs'].shape}")
    print("=" * 60)
