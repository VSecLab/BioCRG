...
def kmeans_with_outlier_detection(df: pd.DataFrame, K: int):
    """
    Perform KMeans clustering and label smaller clusters as outliers.
    ...
    # Keep only the first x clusters (largest ones)
    clusters_to_keep = [cluster_id for cluster_id, _ in sorted_clusters[:x]]
    
    # Label the remaining clusters as outliers (-1)
    for i, label in enumerate(labels):
        if label not in clusters_to_keep:
            labels[i] = -1
...
def grid(df: pd.DataFrame):
    ...
    for i in k_to_test:
        ...
        # goal: minimize inertia to have compact clusters and low number of clusters (trade-off)
