...
def create_suffix_tree(clustering_algorithm, processed_data_path, result_directory, 
                      target_activity, scaler, threshold, k_value, l_value, pmin=0.000001, 
                      gamma_min=0.0, eps_suffix=0.0):
    """
    Create suffix tree for kmeans clustering.
...
def compute_sequences_and_probabilities(lsv_users, log_df, adj_df, state_mapping, suffix_tree, 
    ...
    converted_sequences = ust.convert_sequences_with_mapping(state_mapping, sequences_dict)
...
def run_model_suffix_pipeline(
    ...
    # Create suffix tree (only for kmeans)
    suffix_tree, state_mapping = create_suffix_tree(
        clustering_algorithm, processed_data_path, result_directory,
        target_activity, scaler, threshold, k_value, l_value, pmin, gamma_min, eps_suffix
    )
    ...
    df_sequence_ghost_end = compute_sequences_and_probabilities(
        lsv_users, log_df, adj_df, state_mapping, suffix_tree,
        l_value, ghost_exp, test_activity
    )
